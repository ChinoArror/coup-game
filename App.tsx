...
export default function App() {
  ...
  useEffect(() => {
    ...
    const processAI = async () => {
      ...
      if (aiResponders.length > 0) {
         ...
         } else if (decision.type === 'LOSE_CARD' && decision.payload?.cardToLose) {
             setGameState(prev => prev ? handleLoseCard(prev, responderId, decision.payload.cardToLose) : null);
         } else {
  ...
  const incomingAction = gameState.pendingAction?.action;
  const isTargeted = gameState.pendingAction?.targetId === 'human';

  const canBlockDuke = isActionPhase && incomingAction === ActionType.FOREIGN_AID;
  const canBlockContessa = isActionPhase && incomingAction === ActionType.ASSASSINATE && isTargeted;
  const canBlockCaptain = isActionPhase && incomingAction === ActionType.STEAL && isTargeted;
  const canBlockAmbassador = isActionPhase && incomingAction === ActionType.STEAL && isTargeted;
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {opponents.map(opp => (
              <div key={opp.id} className={`relative p-4 rounded-xl border-2 transition-all ${opp.id === gameState.players[gameState.currentPlayerIndex].id ? 'border-amber-500 bg-amber-500/5 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'border-slate-800 bg-slate-900'}`}>
                {aiThinking === opp.name && (
          ...
          <div className={`p-6 rounded-2xl border-2 transition-all ${isMyTurn ? 'border-amber-500 bg-slate-900/80' : 'border-slate-800 bg-slate-900'}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                {isMyTurn && gameState.phase === GamePhase.TURN_START && !human.isEliminated && (
                    <>
                        <ActionButton label="Income (1)" onClick={() => onAction(ActionType.INCOME)} sub="Safe" />
                        <ActionButton label="Foreign Aid (2)" onClick={() => onAction(ActionType.FOREIGN_AID)} sub="Blockable by Duke" />
                        <ActionButton label="Tax (3)" onClick={() => onAction(ActionType.TAX)} sub="Claims Duke" color="purple" />
                        <ActionButton label="Exchange" onClick={() => onAction(ActionType.EXCHANGE)} sub="Claims Ambassador" color="blue" />
                        <ActionButton label="Steal (2)" onClick={() => {
                            // Simple prompt for target
                            const target = opponents.find(o => !o.isEliminated); // MVP target selection
                            if(target) onAction(ActionType.STEAL, target.id);
                        }} sub="Claims Captain" color="green" />
                        <ActionButton label="Assassinate (3)" disabled={human.coins < 3} onClick={() => {
                            const target = opponents.find(o => !o.isEliminated);
                            if(target) onAction(ActionType.ASSASSINATE, target.id);
                        }} sub="Claims Assassin" color="red" />
                        <ActionButton label="Coup (7)" disabled={human.coins < 7} onClick={() => {
                             const target = opponents.find(o => !o.isEliminated);
                             if(target) onAction(ActionType.COUP, target.id);
                        }} sub="Unblockable" color="red" strong />
                    </>
                )}
                ...
                {canRespond && (
                    <>
                        {/* Always allow Challenge if pending action or block */}
                        {(isActionPhase || isBlockPhase) && (
                 ...
                 <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                     {gameState.logs.slice().reverse().map((log) => (
                         <div key={log.id} className={`text-sm p-2 rounded ${
                             log.type === 'alert' ? 'bg-amber-900/20 text-amber-200 border-l-2 border-amber-500' :
